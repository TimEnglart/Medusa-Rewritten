...
export default class AutoRoleCommand extends ExtendedClientCommand {
	constructor(commandHandler: CommandHandler) {
		...
		this.clientPermissionsRequired = ['MANAGE_ROLES'];
	...
	protected async Run(message: Message, ...args: string[]): Promise<ICommandResult | void> {
		...
		await message.channel.send(RichEmbedGenerator.successEmbed(`Updated Auto Role For New Memebers`, `When a New User Joins They Will Be Given the Role: ${role}`));
	}
}